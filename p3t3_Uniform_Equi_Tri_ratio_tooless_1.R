#Efficiency of Uniform design with p=3, t=3 and n=6 with sigma_1^2/sigma_2^2=0.001
library(MASS)
library(pracma)
...
T_d1=matrix(c(1, 0, 0, 0, 1, 0, 0, 0, 1),nrow=p,ncol=t,byrow=T)
T_d2=matrix(c(0, 1, 0, 0, 0, 1, 1, 0, 0),nrow=p,ncol=t,byrow=T)
T_d3=matrix(c(0, 0, 1, 1, 0, 0, 0, 1, 0),nrow=p,ncol=t,byrow=T)
T_d4=matrix(c(1, 0, 0, 0, 1, 0, 0, 0, 1),nrow=p,ncol=t,byrow=T)
T_d5=matrix(c(0, 1, 0, 0, 0, 1, 1, 0, 0),nrow=p,ncol=t,byrow=T)
T_d6=matrix(c(0, 0, 1, 1, 0, 0, 0, 1, 0),nrow=p,ncol=t,byrow=T)

#Combining period versus direct treatment effect incidence matrices for all subjects
T_d=rbind(T_d1, T_d2, T_d3, T_d4, T_d5, T_d6)
...
write_xlsx(data,"D:/Special Issue/p3t3_Uniform_Equi_Tri_ratio_tooless_1.xlsx")

#Calling library readxl
library(readxl)

p3t3_Uniform_Equi_Tri_ratio_tooless_1=read_excel("D:/Special Issue/p3t3_Uniform_Equi_Tri_ratio_tooless_1.xlsx")

min(p3t3_Uniform_Equi_Tri_ratio_tooless_1$Efficiency) #Minimum efficiency


max(p3t3_Uniform_Equi_Tri_ratio_tooless_1$Efficiency) #Maximum efficiency
